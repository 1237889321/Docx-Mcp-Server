...
async function extractText(file_path: string) {
  ...
  return {
    ...
    word_count: result.value.split(/\s+/).filter((word: string) => word.length > 0).length,
  }
}

async function convertToHtml(file_path: string, include_styles: boolean = true) {
  const absolutePath = path.resolve(file_path)
  if (!fs.existsSync(absolutePath)) {
    throw new Error(`File not found: ${absolutePath}`)
  }
...
async function analyzeStructure(file_path: string) {
  const absolutePath = path.resolve(file_path)
  if (!fs.existsSync(absolutePath)) {
    throw new Error(`File not found: ${absolutePath}`)
  }
  ...
  const headings = (html.match(/<h[1-6][^>]*>.*?<\/h[1-6]>/gi) || []).map((h: string) => ({
  ...
  return {
    document_stats: {
      ...
      total_words: text.split(/\s+/).filter((word: string) => word.length > 0).length,
      total_paragraphs: paragraphs,
      total_headings: headings.length,
    },
    structure: {
      headings,
      heading_levels: [...new Set(headings.map((h: any) => h.level))].sort(),
    },
    formatting: {
...
async function extractImages(file_path: string, output_dir?: string) {
  const absolutePath = path.resolve(file_path)
  if (!fs.existsSync(absolutePath)) {
    throw new Error(`File not found: ${absolutePath}`)
  }
  ...
  const result = await mammoth.convertToHtml({ path: absolutePath }, options)
  const images = (result.value.match(/<img[^>]*>/gi) || []).map((img: string) => {
...
async function convertToMarkdown(file_path: string) {
  const absolutePath = path.resolve(file_path)
  if (!fs.existsSync(absolutePath)) {
    throw new Error(`File not found: ${absolutePath}`)
  }
  ...
  return {
    markdown,
    word_count: markdown.split(/\s+/).filter((word: string) => word.length > 0).length,
    messages: htmlResult.messages,
  }
}
...
if (useHttp) {
  ...
  app.post('/extract_text', upload.single('file'), async (req: Request, res: Response) => {
  ...
  app.post('/convert_to_html', upload.single('file'), async (req: Request, res: Response) => {
    const tempFilePath = (req as any).file?.path
    if (!tempFilePath) {
      return res.status(400).json({ error: 'No file uploaded' })
    }
  ...
  app.post('/analyze_structure', upload.single('file'), async (req: Request, res: Response) => {
    const tempFilePath = (req as any).file?.path
    if (!tempFilePath) {
      return res.status(400).json({ error: 'No file uploaded' })
    }
  ...
  app.post('/extract_images', upload.single('file'), async (req: Request, res: Response) => {
    const tempFilePath = (req as any).file?.path
    if (!tempFilePath) {
      return res.status(400).json({ error: 'No file uploaded' })
    }
  ...
  app.post('/convert_to_markdown', upload.single('file'), async (req: Request, res: Response) => {
    const tempFilePath = (req as any).file?.path
    if (!tempFilePath) {
      return res.status(400).json({ error: 'No file uploaded' })
    }
